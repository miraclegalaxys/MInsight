...
def plot_graph(accuracy_df, predicted_attack): # สร้างฟังก์ชัน plot_graph ที่รับพารามิเตอร์ 2 ตัวคือ accuracy_df, predicted_attack
    while True:  #เริ่มลูปอย่างต่อเนื่อง
        ...
        if plot_gra == 'y': # ถ้าผู้ใช้ต้องการพล็อตกราฟ 
            ...
            plt.tight_layout()
            plt.ion() # กำหนดให้กราฟแสดงอยู่บนหน้าต่างเดียวกันกับโปรแกรม
            plt.show(block=False) # แสดงกราฟ
            save_graph = input("คุณต้องการบันทึกกราฟหรือไม่? (y/n): ") # รับค่าจากผู้ใช้ว่าต้องการบันทึกกราฟหรือไม่
            if save_graph == 'y': # ถ้าผู้ใช้ต้องการบันทึกกราฟ
                plt.savefig('Prediction_Accuracy.png') # บันทึกรูปภาพเป็นไฟล์ 'Prediction_Accuracy.png'
                print("กราฟถูกบันทึกเรียบร้อยแล้ว") # แสดงข้อความเพื่อแจ้งให้ผู้ใช้ทราบว่ากราฟถูกบันทึกเรียบร้อยแล้ว
                break # ออกจากลูป
        elif plot_gra == 'n':
            break  # ออกจากลูป
        else:
